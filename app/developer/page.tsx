...
export default function DeadDropHunter() {
  ...
  const isUnlocked = manualUnlock || distance <= unlockThreshold;
  ...
  if (!isUnlocked) {
    return (
      <div className="min-h-screen bg-white text-zinc-900 font-mono flex items-center justify-center p-4 sm:p-6">
        <div className="card-field w-full max-w-md space-y-4 border-2 border-black bg-white">
          <div className="text-center space-y-2">
            <span className="text-xs tracking-widest uppercase font-bold text-zinc-600">Developer Access</span>
            <div className="divider-thick" />
            <p className="text-sm text-zinc-700">Enter the password to reveal the dashboard.</p>
          </div>
          <input
            type="password"
            placeholder="Enter password"
            onKeyDown={(e) => {
              if (e.key === 'Enter' && e.currentTarget.value === 'anjal@123') {
                setManualUnlock(true);
              }
            }}
